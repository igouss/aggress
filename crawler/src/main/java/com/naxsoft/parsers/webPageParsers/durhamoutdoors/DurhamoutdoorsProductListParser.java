...
public class DurhamoutdoorsProductListParser extends AbstractWebPageParser {
    ...
    private DurhamoutdoorsProductListParser(HttpClient client) {
        this.client = client;
    }
    ...
    public boolean canParse(WebPageEntity webPage) {
        return webPage.getUrl().contains("durhamoutdoors.ca") && webPage.getType().equals("productList");
    }

    @Override
    public void start() throws Exception {
        super.start();
        vertx.eventBus().consumer("durhamoutdoors.ca/productList", getParseRequestMessageHandler());
    }
}