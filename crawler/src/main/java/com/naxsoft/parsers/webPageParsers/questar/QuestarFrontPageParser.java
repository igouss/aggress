...
class QuestarFrontPageParser extends AbstractWebPageParser {
    ...
    private Flux<WebPageEntity> parseDocument(DownloadResult downloadResult) {
        ...
        if (document != null) {
            ...
            for (Element e : elements) {
                ...
                WebPageEntity webPageEntity = WebPageEntity.legacyCreate(downloadResult.getSourcePage(), "", "tmp", href, e.text());
    ...
    private Flux<WebPageEntity> parseSubPages(DownloadResult downloadResult) {
        ...
        if (document != null) {
            ...
            for (Element e : elements) {
                WebPageEntity webPageEntity = WebPageEntity.legacyCreate(downloadResult.getSourcePage(), "", "productList", e.attr("abs:href"), downloadResult.getSourcePage().getCategory());
                LOGGER.info("Product page listing={}", webPageEntity.getUrl());
                result.add(webPageEntity);
            }
            // add product page
            ...
            for (Element e : elements) {
                WebPageEntity webPageEntity = WebPageEntity.legacyCreate(downloadResult.getSourcePage(), "", "productPage", e.attr("abs:href"), downloadResult.getSourcePage().getCategory());
                LOGGER.info("Product page listing={}", webPageEntity.getUrl());
                result.add(webPageEntity);
            }
        }
