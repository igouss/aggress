...
class FrontierfirearmsProductListParser extends AbstractWebPageParser {
    ...
    public FrontierfirearmsProductListParser(MetricRegistry metricRegistry, HttpClient client) {
    ...
    private Flux<WebPageEntity> parseDocument(DownloadResult downloadResult) {
        ...
        if (document != null) {
            ...
            if (select != null && !select.isEmpty()) {
                WebPageEntity webPageEntity = WebPageEntity.legacyCreate(downloadResult.getSourcePage(), "", "productList", select.attr("abs:href"), downloadResult.getSourcePage().getCategory());
